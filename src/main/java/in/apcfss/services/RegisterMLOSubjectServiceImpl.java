...
import in.apcfss.repositories.RegisterMLOSubjectRepo;
...
@Service
public class RegisterMLOSubjectServiceImpl implements RegisterMLOSubjectService {
	...
	@Autowired
	RegisterMLOSubjectRepo MLOsubRepo;

	@Override
	public List<Map<String, Object>> getDesignationList(String deptCode) {
		List<Map<String, Object>> data =null;
		try {
			data=MLOsubRepo.getDesignationList(deptCode);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override
	public List<Map<String, Object>> getRegisterMLOSubjectList(String deptCode) {
		List<Map<String, Object>> data =null;
		try {
			data=MLOsubRepo.getRegisterMLOSubjectList(deptCode);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override
	public String saveMloSubjectDetails(Authentication authentication, HCCaseStatusAbstractReqBody abstractReqBod,
		...
		String subjectDesc=abstractReqBod.getSubjectDesc();
		...
		try {
			...
			if ( mobileNo.trim().length() == 10 && aadharNo.trim().length() == 12) {
				...
				status=MLOsubRepo.insert_mlo_subject_details(deptCode,designationId,employeeId,mobileNo,emailId,aadharNo,userId,request.getRemoteAddr(),subjectDesc );

				if (status == 1){
					status=MLOsubRepo.insert_usersMLO(userId,request.getRemoteAddr(),deptId,deptCode,employeeId,designationId );

					status+=MLOsubRepo.insert_user_rolesMLO(emailId);
					...
					if(status>1) {
						...
						sMs="Mid Level Officer (Legal Sub) details Registered & Login Credentails created successfully. Login details sent to MLO Registered Mobile no.";
					}
					else {
						sMs="Error:Mid Level Officer (Legal Sub) not Registered due to wrong data submitted or Already registered.";
					}
				}else {
					sMs="Error: Mid Level Officer (Legal Sub) not Registered due to wrong data submitted or Already registered";
				}
			...
			e.printStackTrace();
		    sMs = "Error: Invalid input or unexpected error occurred while saving MLO Subject Details.";
		}
		return sMs;
	}
