...
@Service
public class HCCaseStatusAbstractServiceImpl implements HCCaseStatusAbstractService {
	...
	@Override
	public List<Map<String, Object>> getLegacyReportList(String roleId, String userId,
			UserDetailsImpl userPrincipal,HCCaseStatusAbstractReqBody abstractReqBody) {
		String ConditionService = "", condition = "", sqlCondition = "", sql = "";
		try {
			if ((roleId.equals("6"))) {
				if (userId.equals("gp-services1@ap.gov.in") || userId.equals("gp-services2@ap.gov.in")
						|| userId.equals("gp-services3@ap.gov.in") || userId.equals("gp-services4@ap.gov.in")) {
					ConditionService = " and category_service='SERVICE'";
				}
				...
				if (userId.equals("public-prosecutor@ap.gov.in")) {
					...
					sqlCondition += " and (type_name_reg like '%CRL%'  or type_name_reg='RT')  and type_name_reg!='TRCRLA' ";
				}
			...
			sql = "select x.reporting_dept_code as deptcode, upper(d1.description) as description,sum(total_cases) as total_cases,sum(withsectdept) as withsectdept,sum(withmlo) as withmlo,sum(withhod) as withhod,sum(withnodal) as withnodal,sum(withsection) as withsection, sum(withdc) as withdc, sum(withdistno) as withdistno,sum(withsectionhod) as withsectionhod, sum(withsectiondist) as withsectiondist, sum(withgpo) as withgpo, sum(closedcases) as closedcases, sum(goi) as goi, sum(psu) as psu, sum(privatetot) as privatetot  from ("
					...
					+ " " + "where d.display = true " + ConditionService + "  " + sqlCondition;
	...
	public List<Map<String, Object>> getHODwisedetails(UserDetailsImpl userPrincipal, String deptId, String deptName,HCCaseStatusAbstractReqBody abstractReqBody) {
		String ConditionService = "", condition = "", sqlCondition = "", sql = "",sqlCondition1="";
		...
		try {
			...
			sql = "select a.dept_code as deptcode , upper(d.description) as description,count(*) as total_cases, "
					...
					+ "sum(case when case_status=98 and coalesce(ecourts_case_status,'')!='Closed' then 1 else 0 end) as privatetot "
					+ "from ecourts_case_data a " + "inner join dept_new d on (a.dept_code=d.dept_code) "+condition+" "
					+ "where d.display = true "+sqlCondition1+"  and (d.reporting_dept_code='" + deptId + "' or a.dept_code='" + deptId
					+ "') " + sqlCondition + "group by a.dept_code , d.description order by 1";

			
			
			Map<String, Object> map = new HashMap<>();
			map.put("HEADING", "HOD Wise High Court Cases Abstract Report for " + deptName);
			map.put("data",jdbcTemplate.queryForList(sql));
			dataa.add(map);
			
			System.out.println("legacy SQL:" + sql);
			}catch (Exception e) {
	...
	public List<Map<String, Object>> getHCCgetCasesList(UserDetailsImpl userPrincipal,String deptCode, String deptName, String caseStatus,  String reportLevel,HCCaseStatusAbstractReqBody abstractReqBody) {
		...
		try {
			...
			if (!caseStatus.equals("")) {
				...
				if (caseStatus.equals("incomeTax")) {
					sqlCondition = " and (case_status=94 or coalesce(ecourts_case_status,'')='Closed') ";
					heading += " Income Tax ";
				}
				if (caseStatus.equals("centralTax")) {
					sqlCondition = " and (case_status=95 or coalesce(ecourts_case_status,'')='Closed') ";
					heading += " Central Tax ";
				}
			...
			if (userId.equals("public-prosecutor@ap.gov.in")) {
				...
				sqlCondition += " and e.gp_id='"+userId+"' and (type_name_reg like '%CRL%'  or type_name_reg='RT')  and type_name_reg!='TRCRLA' ";
			}
			...
			sql+= "  coalesce(trim(a.scanned_document_path),'-') as scanned_document_path1, b.orderpaths,"
			...
			if (caseStatus.equals("withDistSec")) {
				sql+=" left join nic_data nda on (a.dept_code=substr(nda.global_org_name,1,5) and a.assigned_to=nda.email and nda.is_primary='t' ) "; //and coalesce(a.dist_id,'0')=coalesce(nda.dist_id,'0')
			}
			else if (caseStatus.equals("withHODSec") || caseStatus.equals("withSDSec")) {
				sql+=" left join nic_data nda on (a.dept_code=substr(nda.global_org_name,1,5) and a.assigned_to=nda.email and nda.is_primary='t') ";
			}
			...
			System.out.println("HCCgetCasesList SQL:" + sql);
		}catch (Exception e) {
