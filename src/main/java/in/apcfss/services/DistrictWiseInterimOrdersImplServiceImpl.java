...
@Service
public class DistrictWiseInterimOrdersImplServiceImpl implements DistrictWiseInterimOrdersImplService {
	...
	@Override
	public List<Map<String, Object>> getInterimCASESLIST(Authentication authentication, HCCaseStatusAbstractReqBody abstractReqBody) {
		...
		try {
			...
			sql = "select a.*, b.orderpaths , prayer, ra.address " + " from ecourts_case_data a "
					...
					+ " from "
					+ " (select cino, order_document_path,order_date,order_details||' Dt.'||to_char(order_date,'dd-mm-yyyy') as order_details from ecourts_case_interimorder where order_document_path is not null"
	...
	public Map<String, Object> getCaseStatusUpdate(Authentication authentication,
		...
		try {
			if (cIno != null && !cIno.equals("")) {
				...
				List<Map<String, Object>> data_final = assignedCasesToSectionRepo.getInterimData(cIno);
				...
				System.out.println("data sql data------>"+sql);
				map.put("OLCMSCASEDATA", data);
				...
				if (data != null && !data.isEmpty()) {
					...
					if (data_final != null && !data_final.isEmpty() && data_final.size() > 0) {
						...
						map.put("interim_order", (data_final.get(0).get("order_document_path")).toString());
					}
		...
		try {
			if (cIno != null && !cIno.equals("")) {
				...
				if (abstractReqBody.getEcourtsCaseStatus()!= null
						&& abstractReqBody.getEcourtsCaseStatus().toString().equals("interim")) {
					...
					if (a > 0) {
						...
						&& abstractReqBody.getEcourtsCaseStatus().toString().equals("appeal_interim")) {

					if (abstractReqBody.getAppealFileCopy() != null
							&& !abstractReqBody.getAppealFileCopy().toString().equals("")) {

						System.out.println("getAppealFileCopy is" + abstractReqBody.getAppealFileCopy());
					...
					if (a > 0) {
						...
						&& abstractReqBody.getEcourtsCaseStatus().toString().equals("dismissed_interim")) {
