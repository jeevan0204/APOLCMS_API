...
@Service
public class YearwisePendingReportServiceImpl implements YearwisePendingReportService {

	@Autowired
	JdbcTemplate jdbcTemplate;

	@Override
	public List<Map<String, Object>> getYearWisePendingCasesReport(Authentication authentication, String fromDate,
		...
		String dist_table="";
		dist_table = getTableName(String.valueOf(distId));
		try {
			...
			if(section_code!=null && section_code.equals("N"))
			{
				...
				sql="select fullname_en,global_org_name,designation_name_en,mobile1,b.assigned_to as email,employee_id as emp_id ,count(distinct b.ack_no)  as total ,"
						...
						+ " (count(distinct b.ack_no)-(sum(case when b.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10 then 1 else 0 end) )) balance  , "
						+ " (count(distinct b.ack_no)-(sum(case when b.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10 then 1 else 0 end) )) net_balance  from section_officer_details a "
						...
						+ " inner join "+dist_table+" nd on (a.emailid =nd.email) "  //a.employeeid=c.employee_id and
				...
				sql="select fullname_en,global_org_name,designation_name_en,mobile1,email,employeeid as emp_id,count(distinct b.cino)  as total ,"
						+ " sum(case when b.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10  then 1 else 0 end) as counter_uploaded, "
						+ " (count(distinct b.cino)-(sum(case when b.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10 then 1 else 0 end) )) balance  , "
						+ " (count(distinct b.cino)-(sum(case when b.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10 then 1 else 0 end) )) net_balance  "
	...
	@Override
	public List<Map<String, Object>> getCasesListSectionOfficerYear(Authentication authentication,  String section_code, String emailId, String ids, String date){
		String sql = null, sqlCondition = "", part="" ;

		try {

			if(ids.equals("COUNTER")) {
			...
			}else if(ids.equals("BALANCE")) {
				part=" and a.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10";
			}else if(ids.equals("NETBAL")){
				part=" and a.ecourts_case_status='Pending' and counter_filed_document is not null and length(counter_filed_document)>10";
