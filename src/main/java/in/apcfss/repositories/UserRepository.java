...
public interface UserRepository extends JpaRepository<UsersMst, String>{
	...
	@Query(nativeQuery = true, value = "SELECT userid,user_type, password, password_text, dept_code, dist_id FROM users WHERE userid = :userId")
	List<Map<String, Object>> getUserByUsername(@Param("userId") String userId);


	@Query(nativeQuery = true, value = "SELECT userid,user_type, password, password_text, dept_code, dist_id FROM users WHERE userid =:username and password =:password")
